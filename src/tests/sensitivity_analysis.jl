...
# ── Baseline parameters ───────────────────────────────────────────────────────
const BASE_TIMECOST    = 500 / 60
const BASE_PCOSTSHIFT  = 250
const BASE_HANDLING    = 7
const BASE_NUM_OPS     = 1
...
function plot_sensitivity_results(deck_name, all_sweep_results, save_prefix)
    ...
    for (sw, (Δobj_means, Δobj_cis, Δcargo_means, Δcargo_cis)) in zip(SWEEPS, all_sweep_results)
        ...
        base_idx = findfirst(==(sw.baseline_value), sw.values)
    ...
    fig   = plot(plts..., layout = (2, 2), size = (1200, 800), left_margin = 12mm)
    ...
    for sw in SWEEPS
        ...
        for v in sw.values
            ...
            objs, cargos, shifts = run_on_instances(deck, sw.inst_fn(test_instances, v);
                                                    sw.kwargs_fn(v)...)
            push!(all_objs,        objs)
            push!(all_cargos,      cargos)
            push!(all_shifts,      shifts)
        ...
        base_idx    = findfirst(==(sw.baseline_value), sw.values)
        base_obj    = mean(all_objs[base_idx])
        base_cargo  = mean(all_cargos[base_idx])
        base_shifts = mean(all_shifts[base_idx])
        ...
        for i in eachindex(sw.values)
            Δobj_i   = 100 .* (all_objs[i]   .- base_obj)   ./ abs(base_obj)
            Δcargo_i = 100 .* (all_cargos[i] .- base_cargo) ./ max(base_cargo, 1.0)
