using Random
include(joinpath(pwd(),"src/tests/min_shifts.jl"))
include(joinpath(pwd(),"src/Heuristics/priorityQ.jl"))

# Destroy operators
function get_neighbors(deck,i,j) # Helper func.for some of the neighbor funcs. 
    neighs = []
    h,w = size(deck)

    if i != 1
        push!(neighs, deck[i-1,j])
    end
    if j != 1
        push!(neighs, deck[i,j-1])
    end 
    if i!=h 
        push!(neighs, deck[i+1,j])
    end
    if j!=w
        push!(neighs, deck[i,j+1])
    end
    return neighs
end

function destroy_neighbor_basket(deck, cargo_on, basket;xi=0.2)
    deck = copy(deck)
    cargo_on = copy(cargo_on)
    h,w = size(deck)
    
    
    L = []
    Lloc = []

    n_cargo = 0
    for (i, row) in enumerate(eachrow(deck))
        for (j, slot) in enumerate(row)
            if slot > 2
                n_cargo +=1
            end
        end
    end

    
    for (i, row) in enumerate(eachrow(deck))
        for (j, slot) in enumerate(row)
            if slot > 2
                if !(slot in get_neighbors(deck,i,j))
                    push!(Lloc,[i,j])
                end
            end

        end
    end

    for (count, loc) in enumerate(shuffle!(Lloc))
        if count > xi*n_cargo

            return deck,L, cargo_on, basket
        else 
            push!(L, cargo_on[loc[1], loc[2]])
            cargo_on[loc[1], loc[2]] = nothing
            deck[loc[1], loc[2]] = 1
        end
    end
    return deck, L, cargo_on, basket
end

function destroy_neighbor_basket_v2(deck, cargo_on, basket;xi=0.2)
    deck = copy(deck)
    cargo_on = copy(cargo_on)
    h,w = size(deck)
    
    L = []
    init_cargo_length = length(findall(x->x>2,deck))

    for i in 1:4
        cargo_locs = findall(x->x>2,deck)
        
        for loc in cargo_locs
            slot = deck[loc[1],loc[2]]
            
            if count(x->x==slot, get_neighbors(deck,loc[1],loc[2])) == i-1
                push!(L,cargo_on[loc[1],loc[2]])
                cargo_on[loc[1],loc[2]] = nothing
                deck[loc[1],loc[2]] = 1
                if length(L) > init_cargo_length*xi
                    return deck, L, cargo_on, basket
                end
            end
        end
    end

    while length(L) <= init_cargo_length*xi
        carg = findall(x->x>2,deck)
        if isempty(carg)
                return deck, L, cargo_on, basket
        else
            s = rand(carg)
            i,j = s[1],s[2]
            push!(L,cargo_on[i,j])
            cargo_on[i,j] = nothing
            deck[i,j] = 1
        end
    end


    
   
    return deck, L, cargo_on, basket
end

function destroy_area_basket(deck, cargo_on, basket; xi=0.1)
    deck = copy(deck)
    cargo_on = copy(cargo_on)

    h, w = size(deck)

    n_cargo = count(!isnothing, cargo_on)
    cargo_positions = findall(!isnothing, cargo_on)
    target = ceil(Int, n_cargo * xi)
    
    cargo2place = Any[]
    if isempty(cargo_positions)
        return deck, cargo2place, cargo_on, basket
    end
    start_loc = rand(cargo_positions)
    start_i, start_j = start_loc[1], start_loc[2]
    area_height = 0
    area_width = 0

    while length(cargo2place) < target && area_height ≤ h && area_width ≤ w

        # Expand crater area
        for i in start_i:(start_i + area_height)
            for j in start_j:(start_j + area_width)

                if i < 1 || i > h || j < 1 || j > w
                    continue
                end

                if deck[i, j] > 2 && !isnothing(cargo_on[i, j])
                    push!(cargo2place, cargo_on[i, j])
                    deck[i, j] = 1
                    cargo_on[i, j] = nothing
                end

                if length(cargo2place) >= target
                    break
                end
            end
                
            if length(cargo2place) >= target
                break
            end
            
        end

        # Randomly expand crater
        if rand() > 0.5
            area_height += 1
        else
            area_width += 1
        end
        
    end

    return deck, cargo2place, cargo_on, basket
end

function destroy_random_basket(deck, cargo_on, basket;xi=0.2)
    deck = copy(deck)
    h,w = size(deck)
    cargo_on = copy(cargo_on)

    n_cargo = 0
    for (i, row) in enumerate(eachrow(deck))
        for (j, slot) in enumerate(row)
            if slot > 2
                n_cargo +=1
            end
        end
    end

    L = []
    while length(L) < xi*n_cargo
        i = rand(1:h)
        j = rand(1:w)

        if deck[i,j] > 2
            push!(L,cargo_on[i,j])
            deck[i,j] = 1
            cargo_on[i,j] = nothing
        end
    end
    return deck, L, cargo_on, basket
end

function destroy_port_basket(deck, cargo_on, basket;xi=0.2)
    if isempty(unique(deck)[unique(deck).>2])
        port2rem = 3
    else
        if rand() < 0.5
            port2rem = rand(unique(deck)[unique(deck).>2])
        else
            port2rem = rand(unique(deck)[unique(deck).>2],2)
        end
    end

    deck = copy(deck)
    cargo_on = copy(cargo_on)

    h,w = size(deck)

    n_cargo = 0
    for (i, row) in enumerate(eachrow(deck))
        for (j, slot) in enumerate(row)
            if in(slot,port2rem)
                n_cargo +=1
            end
        end
    end

    L = []
    while length(L) < xi*n_cargo 
        i = rand(1:h)
        j = rand(1:w)

        if in(deck[i,j], port2rem)
            push!(L,cargo_on[i,j])
            deck[i,j] = 1
            cargo_on[i,j] = nothing
        end
    end
    return deck, L, cargo_on, basket
end

function destroy_shifting_cost_basket(deck, cargo_on, basket;xi=0.2)
    deck = copy(deck)
    cargo_on = copy(cargo_on)
    _,V = shortest_path_like_hansen(deck)
    h,w = size(deck)
    
    n_cargo = 0
    for (i, row) in enumerate(eachrow(deck))
        for (j, slot) in enumerate(row)
            if slot > 2
                n_cargo +=1
            end
        end
    end
    
    if isempty(V)
        # Randomly sample cargo positions as blockers
        cargo_positions = []
        for i in 1:h
            for j in 1:w
                if deck[i,j] > 2
                    push!(cargo_positions, [i,j])
                end
            end
        end
        n_selected_blockers = max(1, round(Int, xi * n_cargo / 2))
        n_selected_blockers = min(n_selected_blockers, length(cargo_positions))
        selected_blockers = shuffle!(cargo_positions)[1:n_selected_blockers]
    else
        blockers = [[div(num[2], w)+1,num[2]%w +1] for num in V]
        n_selected_blockers = max(1, round(Int, (xi/2)*length(blockers)))
        selected_blockers = [popfirst!(shuffle!(blockers)) for n in 1:n_selected_blockers]
    end
    
    rad = length(selected_blockers)
    Lloc =[]
    L = []

    for slot in selected_blockers
        push!(Lloc,slot)
        (i,j) = slot
        for skridt in 1:rad
            if i !=1
                push!(Lloc,[i-1, j])
                if j != 1
                    push!(Lloc,[i-1, j-1])
                end
                if j != w
                    push!(Lloc,[i-1, j+1])
                end
            end 
            if i != h
                push!(Lloc,[i+1, j])
                if j != 1
                    push!(Lloc,[i+1, j-1])
                end
                if j != w
                    push!(Lloc,[i+1, j+1])
                end
            end 
            if j !=1
                push!(Lloc,[i, j-1])
            end 
            if j != w
                push!(Lloc,[i, j+1])
            end 
        end
        
    end

    for loc in Lloc
        (i,j) = loc
        if deck[i,j ] > 2
            push!(L, cargo_on[i,j])
            deck[i,j] = 1
            cargo_on[i,j] = nothing
        end
    end
    return deck, L, cargo_on, basket
end

function destroy_bordering(deck, cargo_on, basket;xi=0.2)
    deck = copy(deck)
    cargo_on = copy(cargo_on)
    h,w = size(deck)
    
    L = []
    init_cargo_length = length(findall(x->x>2,deck))
    


    cargo_locs = findall(x->x>2,deck)
    
    for loc in cargo_locs
        slot = deck[loc[1],loc[2]]
        if loc[2] == w
            continue
        end
        if deck[loc[1],loc[2]+1] > slot
            push!(L,cargo_on[loc[1],loc[2]])
            cargo_on[loc[1],loc[2]] = nothing
            deck[loc[1],loc[2]] = 1
            if length(L) > init_cargo_length*xi
                return deck, L, cargo_on, basket
            end
        end
    end


    while length(L) <= init_cargo_length*xi
        carg = findall(x->x>2,deck)
        if isempty(carg)
                return deck, L, cargo_on, basket
        else
 
            cords = rand(carg)

            i,j = cords[1], cords[2]
            push!(L,cargo_on[i,j])
            cargo_on[i,j] = nothing
            deck[i,j] = 1
        end
    end


    
   
    return deck, L, cargo_on, basket
end

function destroy_latest(deck, cargo_on, basket;xi=0.2)
    deck = copy(deck)
    cargo_on = copy(cargo_on)
    h,w = size(deck)
    
    L = []
    init_cargo_length = length(findall(x->x>2,deck))
    
    
    all_cargo = filter(x->!isnothing(x),cargo_on)

    sort!(all_cargo, by= x->x.arr, rev=true)
    
    for carg in all_cargo
        loc = findall(x->x==carg,cargo_on)[1]
        #println(loc[1][2])
        i,j = loc[1],loc[2]

        push!(L,cargo_on[i,j])
        cargo_on[i,j] = nothing
        deck[i,j] = 1
        if length(L) > init_cargo_length*xi
            return deck, L, cargo_on, basket
        end
    end
 
   
    return deck, L, cargo_on, basket
end

function destroy_lanes(deck, cargo_on, basket;xi=0.2)
    deck = copy(deck)
    cargo_on = copy(cargo_on)
    h,w = size(deck)
    L=[]
    init_cargo_length = count(x->!isnothing(x),cargo_on)


    for (i,row) in enumerate(shuffle(eachrow(deck)))
        for (j,slot) in collect(enumerate(reverse(row)))
            if isnothing(cargo_on[i,j])
                continue
            else
                push!(L,cargo_on[i,j])
                cargo_on[i,j] = nothing
                deck[i,j] = 1

                if length(L) > init_cargo_length*xi

                    return deck, L, cargo_on, basket
                end
            end
        end
    end
 
    
    return deck, L, cargo_on, basket
end

# Repairers 
function repair_greedy_basket(deck, cargo2place, cargo_on, basket)  

    cargoDict = Dict()
    scoreList = []
    deck = copy(deck)
    cargo_on = copy(cargo_on)
    cargo = copy(cargo2place)
    h,w = size(deck)

    for (i,c) in enumerate(cargo)
        cargoDict["c$i"] = c
    end

    if rand(1:2)==1
        for (k,v) in cargoDict
            push!(scoreList, [k, v.arr])
        end
        scoreList = sort(scoreList, by = x -> x[2])
    else
        for (k,v) in cargoDict
            push!(scoreList, [k, v.port])
        end
        scoreList = sort(scoreList, by = x -> x[2],rev=true)
    end
    
    placed_cargo = []  # Track what we place
    
    for score in scoreList
        (id,_) = score
        cport = cargoDict[id].port
        carg = cargoDict[id]
        placed = false
        for (j, col) in reverse(collect(enumerate(eachcol(deck))))
            if placed
                continue
            end
            for (i,slot) in enumerate(col)
                if placed
                    continue
                end
                if slot == 1
                    deck[i,j] = cport
                    placed = true
                    cargo_on[i,j] = carg
                    push!(placed_cargo, carg)  # Track this item
                end
            
            end
            
        end
    end
    
    # Remove placed cargo from cargo2place
    for carg in placed_cargo
        filter!(x -> x !== carg, cargo2place)
    end
    
    return deck, cargo_on, basket
end

function repair_neighbor_basket(deck, cargo2place,cargo_on, basket)
    deck = copy(deck)
    cargo_on = copy(cargo_on)


    h,w = size(deck)

    for (i,row) in enumerate(eachrow(deck))
        for (j,slot) in enumerate(row)

            if slot == 1 && !isempty(cargo2place)
                placed = false

                for cargo in shuffle!(copy(cargo2place))
                    if !placed
                        if cargo.port in get_neighbors(deck,i,j)
                            cargo_on[i,j] = cargo
                            deck[i,j] = cargo.port
                            filter!(x->x !== cargo,cargo2place)
                            placed = true

                        end
                    end
                end
                if !placed 
                    c2p = popfirst!(shuffle!(cargo2place))
                    cargo_on[i,j] = c2p
                    deck[i,j] = c2p.port
                    filter!(x->x !== c2p,cargo2place)
                end
            end
        end 
    end
        
    return deck,cargo_on, basket
end

function repair_neighbor_basket_v2(deck, cargo2place,cargo_on, basket)
    deck = copy(deck)
    cargo_on = copy(cargo_on)
    cargo2place = copy(cargo2place)

    h,w = size(deck)
    
    for n in 4:-1:1
        freespots = findall(x->x==1,deck)
        shuffle!(freespots)
        for spot in freespots
            i,j = spot[1],spot[2]
            neighs = get_neighbors(deck,i,j)
            for idx in eachindex(cargo2place)
                carg = cargo2place[idx]
                if count(x->x==carg.port,neighs) == n && deck[i,j] == 1
                    deck[i,j] = carg.port
                    cargo_on[i,j] = carg
                    deleteat!(cargo2place, idx)
                    break
                end
            end
        end
    end
    
    # Place any remaining cargo in available slots
    while !isempty(cargo2place)
        placed_any = false
        for (i,row) in enumerate(eachrow(deck))
            for (j,slot) in enumerate(row)
                if slot == 1 && !isempty(cargo2place)
                    c = popfirst!(cargo2place)
                    cargo_on[i,j] = c
                    deck[i,j] = c.port
                    placed_any = true
                end
            end
        end
        if !placed_any
            error("Repair failed: not enough space for all cargo")
        end
    end

    return deck,cargo_on, basket
end

function repair_neighbor_rand_basket(deck, cargo2place,cargo_on, basket)
    deck = copy(deck)
    cargo_on = copy(cargo_on)


    h,w = size(deck)

    locs = []
    for (i,row) in enumerate(eachrow(deck))
        for (j,slot) in enumerate(row)
            if slot == 1
                push!(locs, [i,j])
            end
        end
    end

    shuffle!(locs)
    for loc in locs
        (i,j) = loc[1], loc[2]
        slot = deck[i,j]

        if !isempty(cargo2place)
            placed = false

            for cargo in shuffle!(copy(cargo2place))
                if !placed
                    if cargo.port in get_neighbors(deck,i,j)
                        cargo_on[i,j] = cargo
                        deck[i,j] = cargo.port
                        filter!(x->x !== cargo,cargo2place)
                        placed = true

                    end
                end
            end

        end
    end

    freespots_remaining = findall(x->x==1, deck)
    for spot in freespots_remaining
        if !isempty(cargo2place)
            c = popfirst!(cargo2place)
            i, j = spot[1], spot[2]
            deck[i,j] = c.port
            cargo_on[i,j] = c
        else
            break
        end
    end

    return deck,cargo_on, basket
end

function repair_placement_basket(deck, cargo2place,cargo_on, basket)
    deck = copy(deck)
    cargo_on = copy(cargo_on)
    g_deck = copy(deck)
    g_deck[g_deck .> 2] .= 1
    m,n = size(deck)
    free_cart = findall(x-> x==1, g_deck)
    free = [(c[1],c[2]) for c in free_cart]
    
    g = get_graph(g_deck)
    goal_slots = get_ramp_loc(g_deck)
    goal_pos = [(loc[1]-1)*n + loc[2] for loc in goal_slots]

    # Optimization: Run dijkstra from EACH GOAL, not from each free position
    # This reduces calls from O(n_free) to O(n_goals), typically 10-100x faster
    all_dists = zeros(m * n)
    for goal in goal_pos
        dsp = dijkstra_shortest_paths(g, goal)
        all_dists .= min.(all_dists, dsp.dists)
    end

    furthest_locs = []
    for loc in free
        pos = (loc[1]-1)*n + loc[2]
        dis = all_dists[pos]
        push!(furthest_locs,[loc,dis])
    end
    sort!(furthest_locs, by = x -> x[2],rev=true)
    sort!(cargo2place, by = x->x.port)
    
    for loc in furthest_locs
        if isempty(cargo2place)
            break  # Changed from return to allow cleanup
        end
        i,j = loc[1][1], loc[1][2]

        if deck[i,j] == 1 && isnothing(cargo_on[i,j] )
            c = popfirst!(cargo2place)
            deck[i,j] = c.port
            cargo_on[i,j] = c
        end

    end
    
    # Place any remaining cargo in first available slots
    if !isempty(cargo2place)
        for (i,row) in enumerate(eachrow(deck))
            for (j,slot) in enumerate(row)
                if slot == 1 && !isempty(cargo2place)
                    c = popfirst!(cargo2place)
                    cargo_on[i,j] = c
                    deck[i,j] = c.port
                end
            end
        end
    end
    
    return deck, cargo_on, basket

end

function repair_random_basket(deck, cargo2place, cargo_on, basket)
    if isempty(cargo2place)
        return deck, cargo_on, basket
    end
    deck = copy(deck)
    h,w = size(deck)

    locs = []
    for (i,row) in enumerate(eachrow(deck))
        for (j,slot) in enumerate(row)
            if slot == 1
                push!(locs, [i,j])
            end
        end
    end

    shuffle!(locs)

    n_to_place = min(length(locs), length(cargo2place))
    for i in 1:n_to_place
        loc = locs[i]
        c2p = cargo2place[i]
        deck[loc[1], loc[2]] = c2p.port
        cargo_on[loc[1], loc[2]] = c2p
    end

    deleteat!(cargo2place, 1:n_to_place)
    return deck,cargo_on, basket
end

function repair_in_basket(deck, cargo2place, cargo_on, basket)
    deck = deepcopy(deck)
    cargo2place = deepcopy(cargo2place)

    cargo_on = deepcopy(cargo_on)
    basket = deepcopy(basket)


    if isempty(cargo2place)
        return deck, cargo_on, basket
    end

    
    in_basket = sort(cargo2place, by = x-> x.arr)[end]
    vec_cargo_on = filter( x -> !isnothing(x), (vec(cargo_on)))
    if !isempty(vec_cargo_on) && in_basket.arr >= sort(vec_cargo_on, by = x -> x.arr)[end].arr
        if rand()<0.5
            filter!(x->x!=in_basket, cargo2place)
            push!(basket, in_basket)
        end
    end

    inner_repair = repair_greedy_basket #rand([repair_random_basket, repair_greedy_basket, repair_neighbor_basket_v2, repair_placement_basket])

    (deck,cargo_on,basket) = inner_repair(deck, cargo2place, cargo_on,basket)

    return deck, cargo_on, basket
end

function repair_out_basket(deck, cargo2place, cargo_on, basket)
    deck = deepcopy(deck)
    cargo2place = deepcopy(cargo2place)
    cargo_on = deepcopy(cargo_on)
    basket = deepcopy(basket)

    if isempty(basket)
        inner_repair = rand([repair_random_basket, repair_greedy_basket, repair_neighbor_basket_v2, repair_placement_basket,])
        (deck,cargo_on,basket) = inner_repair(deck, cargo2place, cargo_on,basket)
        return deck, cargo_on, basket
    end

    out_basket = popfirst!(basket)
    push!(cargo2place, out_basket)

    inner_repair = rand([repair_random_basket, repair_greedy_basket, repair_neighbor_basket, repair_placement_basket])

    (deck,cargo_on,basket) = inner_repair(deck, cargo2place, cargo_on,basket)

    return deck, cargo_on, basket    
end





